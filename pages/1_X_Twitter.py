...
with st.sidebar:
    ...
    if _mode == "api":
        ...
        st.warning(
            "🚧 **GitHub Actions 模式**\n\n"
            "X/Twitter 需要登录态，无法在 GitHub Actions 中运行。\n"
            "请使用 **本地模式** 或 **远程 API 模式**。\n\n"
            "微信搜索不受影响，可正常使用 👉"
        )
    else:
        st.info(
...
with tab_scrape:
    ...
    if _x_mode == "github":
        st.error(
            "⚠️ X/Twitter 搜索需要浏览器登录态，无法在 GitHub Actions 环境中使用。\n\n"
            "解决方案：\n"
            "- **本地运行**: `streamlit run app.py`，首次会弹出浏览器登录\n"
            "- **自建 API**: 部署 Docker 后端，用远程 API 模式\n\n"
            "微信公众号搜索不需要登录，可正常使用 ✅"
        )
        st.stop()
